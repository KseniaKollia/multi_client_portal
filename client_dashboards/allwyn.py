...
# ---------------------------------------------------------
st.set_page_config(
    page_title="DECLUTTERING DASHBOARD",
    layout="wide",
    page_icon="📈"
)
...
st.markdown("""
    ...
    /* 1. ΕΞΑΦΑΝΙΣΗ ΜΟΝΟ ΤΩΝ ΚΟΥΜΠΙΩΝ GITHUB / FORK / SHARE / MENU */
    ...
    /* 2. ΚΡΑΤΑΜΕ ΤΟ HEADER ΔΙΑΦΑΝΕΣ ΚΑΙ ΤΟ ΚΟΥΜΠΙ ΤΟΥ SIDEBAR ΕΝΕΡΓΟ */
...
background_sheet_checker()

try:
    df_opap, _ = get_sheet_data_smart("OPAP ΕΡΩΤΗΣΗ 2000")
    df_stores, _ = get_sheet_data_smart("STORE STATUS (WEEKLY)")
...
with header_col2:
    st.markdown('<div class="header-text-style">Decluttering Dashboard</div>', unsafe_allow_html=True)
...
if not df_filtered.empty:
    ...
    # 2. Κρατάμε ΜΟΝΟ την πιο πρόσφατη εγγραφή για κάθε κατάστημα (Last Response)
    ...
    valid_answers = ["ΝΑΙ", "ΔΕΝ ΥΠΗΡΧΕ ΠΑΛΑΙΟ-ΜΗ ΕΓΚΕΚΡΙΜΕΝΟ ΥΛΙΚΟ", "ΔΕΝ ΥΠΗΡΧΕ ΠΑΛΑΙO-ΜΗ ΕΓΚΕΚΡΙΜΕΝO ΥΛΙΚO"]
    ...
    fig_active.update_layout(
        ...
        legend=dict(
            ...
            traceorder='normal'  # Διασφαλίζει τη σωστή σειρά στο legend
        )
    )
...
# ---------------------------------------------------------
st.markdown("---")
st.subheader("ΑΦΑΙΡΕΘΗΚΕ ΤΥΧOΝ ΤOΠOΘΕΤΗΜΕΝO ΠΑΛΑΙO Η ΜΗ ΕΓΚΕΚΡΙΜΕΝΟ ΥΛΙΚO (ΑΦΙΣΕΣ) ΑΠO ΤO ΚΑΤΑΣΤΗΜΑ ?")
...
if "WEEK_NUM" in df_filtered.columns and "ANSWER" in df_filtered.columns and not df_filtered.empty:
    ...
    color_map = {
        ...
        "ΔΕΝ ΥΠΗΡΧΕ ΠΑΛΑΙO-ΜΗ ΕΓΚΕΚΡΙΜΕΝO ΥΛΙΚO": "#09A1A4",
        "ΟΧΙ": "#115566"
    }
